 ...
 */
static void req_cert_destroy(req_cert_t *reqCert)
{
	...
	while (parser->iterate(parser, &objectID, &object))
	{
		...
		switch (objectID)
		{
			...
			case OCSP_REQ_REQ_CERT:
				reqCert = malloc_thing(req_cert_t);
	...
	private_x509_ocsp_request_t *this)
{
	if (ref_put(&this->ref))
	{
		...
		this->reqCerts->destroy_function(this->reqCerts, (void*)req_cert_destroy);
		...
		switch (va_arg(args, builder_part_t))
		{
			...
			case BUILD_CERT:
				...
				if (cert->get_type(cert) == CERT_X509)
				{
					x509 = (x509_t*)cert;
					reqCert = malloc_thing(req_cert_t);
					reqCert->serialNumber = chunk_clone(x509->get_serial(x509));
					this->reqCerts->insert_last(this->reqCerts, reqCert);
				}
				continue;
